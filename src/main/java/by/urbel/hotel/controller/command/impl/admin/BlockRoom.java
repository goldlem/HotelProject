...
public class BlockRoom implements Command {
    private static final Logger LOGGER = LogManager.getLogger(BlockRoom.class.getName());
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (!user.getRole().equals(UserType.ADMIN)) {
            ...
            try {
                roomService.blockRoom(roomNumber);
                LOGGER.info("Room number {} is blocked",roomNumber);
                response.sendRedirect(request.getContextPath() + PageURL.FIND_ROOMS_PAGE);
            } catch (ServiceException e) {
                LOGGER.error("A server error while blocking room", e);
