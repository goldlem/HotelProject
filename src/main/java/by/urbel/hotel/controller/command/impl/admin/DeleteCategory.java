...
public class DeleteCategory implements Command {
    private static final Logger LOGGER = LogManager.getLogger(DeleteCategory.class.getName());
    private static final String DELETE_ROOMS = "Удалите комнаты данной категории";
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (!user.getRole().equals(UserType.ADMIN)) {
            ...
            try {
                ...
                LOGGER.debug(categoryName + " is category for delete");
                boolean isDeleted = ServiceProvider.getInstance().getCategoryService().deleteRoomCategory(categoryName);
                if (!isDeleted) {
                    request.getSession().setAttribute(RequestAttributes.ERROR, DELETE_ROOMS);
                    LOGGER.error("Category {} is not deleted", categoryName);
                }else {
                    LOGGER.error("Category {} is deleted", categoryName);
                }
                response.sendRedirect(request.getContextPath() + PageURL.FIND_CATEGORIES_PAGE);
            } catch (ServiceException e) {
                LOGGER.error("A server error while deleting category", e);
